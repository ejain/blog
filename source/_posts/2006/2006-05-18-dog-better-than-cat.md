...
[dog](http://www.tucows.com/preview/8291). This tool works similar to cat but has an additional `--lines` option for indicating one or more ranges of lines. A quick look through the documentation reveals that this tool also has some more exciting options such as `--strfry` to shuffle characters within each line and `--oog` to translate the input back to the stone age...

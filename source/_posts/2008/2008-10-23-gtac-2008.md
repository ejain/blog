...
**Marc-Elian Bégin (Six2) on _Using Cloud Computing to Automate Full-Scale System Tests_:** The case was made that Amazon EC2 is of great use for (system-level) testing. The speaker predicts that paravirtualization (e.g. with Xen) will increasingly be replaced with full virtualization (e.g. KVM), even though the latter requires special hardware support (built into most newer processors?).
